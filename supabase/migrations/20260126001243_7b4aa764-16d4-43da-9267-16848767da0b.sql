-- ====================================================================
-- FIX: Timezone-aware comparison for EXPENSE_DUE_TODAY notifications
-- Problem: CURRENT_DATE uses UTC, causing false negatives for BRT users
-- Solution: Convert server timestamp to 'America/Sao_Paulo' timezone
-- ====================================================================
...
DECLARE
  ...
  v_local_today date;
BEGIN
  -- Calculate today's date in São Paulo timezone (BRT/BRST)
  v_local_today := (CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date;

  -- Only trigger for EXPENSE with pending status and due_date = today (local)
  ...
  -- FIXED: Compare against local São Paulo date instead of UTC CURRENT_DATE
  IF NEW.due_date IS NULL OR NEW.due_date::date != v_local_today THEN
    RETURN NEW;
  END IF;
...
-- Update the batch reconciliation function as well
...
BEGIN
  ...
  FOR v_tx IN
    ...
    WHERE t.kind = 'EXPENSE'
      ...
      AND t.due_date::date = v_local_today  -- FIXED: Use local timezone
  ...
  LOOP
    ...
    SELECT id INTO v_existing_id
    FROM public.notifications
    WHERE household_id = v_tx.household_id
      AND dedupe_key = v_dedupe_key
      AND dismissed_at IS NULL
    LIMIT 1;
